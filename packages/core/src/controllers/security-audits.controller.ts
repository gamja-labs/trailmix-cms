...
import { ApiOperation, ApiTags, ApiNotFoundResponse, ApiParam, ApiQuery, ApiBearerAuth } from '@nestjs/swagger';
...
import { RequireAdmin } from '../decorators/index.js';
import { SecurityAuditService } from '../services/index.js';

/**
 * Read-only access to security audit events.
 *
 * Marked {@link RequireAdmin} — restricted to an authenticated `admin` principal. Core ships only
 * the marker; your auth integration enforces it (e.g. `@trailmix-cms/core`'s setup registers
 * a guard that checks the better-auth `admin` plugin's role).
 */
@RequireAdmin()
@ApiBearerAuth()
...
export class SecurityAuditsController {
    ...
    async getSecurityAudits(
        ...
        const audits = await this.securityAuditService.find(filter);
    ...
    async getSecurityAuditById(
        ...
        const audit = await this.securityAuditService.get(auditId);
